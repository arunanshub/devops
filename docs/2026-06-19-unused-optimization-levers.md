...
- **Phase 2 — etcd metrics** ✅ **DONE.** 2a (ansible `k3s-etcd-metrics.yml`) exposes :2381; 2b (gitops) scrapes it via a `VMStaticScrape` + ships etcd rules via `kubeEtcd.enabled`. Image-pull parallelism dropped (k3s kubelet rejects `--max-parallel-image-pulls`). **Phase 3 (Spegel) ← NEXT.**
