...
- **Phase 2 (one rolling pass + paired gitops) — etcd metrics + image-pull parallelism.** ← NEXT
  ...
  - **Status (2026-06-19):** playbook authored at `ansible/playbooks/k3s-etcd-metrics-image-pulls.yml` — handler-based (notify `Restart k3s` + `flush_handlers`), `serial:1`/`max_fail_percentage:0`, two drop-in files, built-in verifies (etcd non-loopback :2381 via `ss`; configz for the two kubelet settings). Lints clean at ansible-lint **production** profile; inventory contract (`control_planes` = cp-1/2/3, `kubernetes_node_name` = live Node name) verified against `tools/ansible-inventory/main.go` + live cluster. **Awaiting operator run** (`just ansible-check` then `just ansible-converge k3s-etcd-metrics-image-pulls`); k3s-via-ansible is operator-run per the safety rule. Pre-flight: eyeball `ansible_host` populated for all 3 CPs and do NOT run during a `tofu apply` (inventory has no per-node empty-IPv6 guard). **2b (gitops kubeEtcd scrape) HELD** until :2381 confirmed live on every node. 2b is de-risked: the `(apiserver|etcd)_.*` drop is scoped to `kubelet.vmScrape` (not a global relabel), so a new `kubeEtcd` job's series survive; and 2381 is plain HTTP (no etcd TLS/certs — those are on 2379/2380/2382), so the scrape uses `scheme: http`.
- **Phase 3 — k3s `embedded-registry: true` (Spegel P2P image mirror).** VERIFIED (k3s docs 2026-06): the flag alone does NOTHING — must ALSO add `mirrors:` entries to `registries.yaml` on every node, either an explicit list (docker.io, registry.k8s.io, ghcr.io, quay.io) OR a single quoted `"*"` wildcard entry to mirror ALL registries (wildcard support added in the March-2024 k3s releases ≥ v1.26.15+k3s1 / 1.27.12 / 1.28.8 / 1.29.3; our v1.35.5+k3s1 is well past it; the asterisk **MUST be quoted**). Decide explicit-list vs wildcard in Phase 3 — src https://docs.k3s.io/installation/registry-mirror#enabling-registry-mirroring. Ports: local registry on 6443 (= our API port, already open), P2P DHT on 5001 (new); nodes reach each other on INTERNAL IPs on 5001+6443. Our inter-node path is the Hetzner private net (10.0.0.x), which the cloud FW does not filter — likely NO firewall change, but verify 5001 node↔node first. Benefit: faster rollouts + Docker-Hub-rate-limit insulation.
